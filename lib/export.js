...
function exportCleanedList(inputFile, deadDomains, redirectDomains, ignoreSimilar = false, removeRedirects = false) {
  ...
  if (redirectDomains && redirectDomains.length > 0) {
    ...
    if (removeRedirects) {
      // --remove-redirects: Always remove redirects (respecting --ignore-similar)
      ...
      console.log(`--remove-redirects: Forcing removal of ${redirectsToRemove.length} redirected domains`);
    } else {
      // Default: only add redirects that aren't similar-domain
      const redirectsToRemove = ignoreSimilar
        ? redirectDomains.filter(r => !isSimilarDomain(r.domain, r.finalDomain))
        : redirectDomains;
    ...
    console.log(`Redirect domains to remove: ${domainsToRemove.size - deadDomains.length}${ignoreSimilar ? ' (ignoring similar-domain redirects)' : ''}`);
